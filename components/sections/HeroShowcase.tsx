...
export function HeroShowcase() {
  return (
    <div className="relative mx-auto w-full max-w-[42rem]">
      ...
      <div className="relative grid gap-4 rounded-[2rem] border border-border/60 bg-panel/60 p-4 shadow-card sm:grid-cols-[minmax(0,1.08fr)_minmax(15rem,0.92fr)] sm:p-5 lg:gap-5 lg:p-6">
        <Link
        ...
        >
          <Card className="relative h-full min-h-[37rem] overflow-hidden rounded-shell bg-panel/94 p-0 shadow-elevated">
            <ProductMockCover
              category="textbooks"
        ...
        <div className="grid gap-4 sm:grid-rows-[auto_1fr]">
          <Link
            href="/book-club"
          ...
          >
            <Card className="relative min-h-[14rem] overflow-hidden bg-panel/94 p-4 sm:min-h-[15rem] sm:p-5">
              <Image
              ...
              <div className="relative rounded-[1rem] border border-white/22 bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.42)_0%,rgba(255,255,255,0.22)_52%,rgba(255,255,255,0.08)_100%)] px-3 py-3 backdrop-blur-[4px]">
                <h3 className="font-display text-[1.8rem] leading-none text-foreground sm:text-[2rem]">Книжный клуб</h3>
                <p className="mt-2 text-[0.95rem] leading-6 text-foreground/80">Встречи и мероприятия</p>
              </div>
            </Card>
          </Link>

          <Card className="overflow-hidden border-brand/15 bg-brand px-5 pb-6 pt-9 text-brand-foreground shadow-soft">
            <div className="mb-3 flex items-center gap-3">
              <span className="flex size-11 items-center justify-center rounded-full bg-brand-foreground/12 text-brand-foreground">
                <StickyNote className="size-5" />
              </span>
              <div>
                <h3 className="font-display text-[1.8rem] leading-none sm:text-[2rem]">Уточнить по телефону</h3>
              </div>
            </div>
            <p className="text-[0.95rem] leading-7 text-brand-foreground/88">Перед визитом можно уточнить наличие нужного товара.</p>
            <Button href={PRIMARY_TEL} variant="secondary" className="mt-5 h-12 w-full justify-center border-white/20 bg-brand-foreground/10 text-brand-foreground hover:bg-brand-foreground/18">
